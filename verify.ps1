...
if ((Test-Path $pluginPath) -and (Test-Path $sourcePluginPath)) {
    ...
    Add-Result "Installed plugin version" ($versionField.Constant -eq "1.0.8") ([string]$versionField.Constant)
}

$logPath = Join-Path $PeakDir "BepInEx\LogOutput.log"
if ($RequireRuntimeLoad -and (Test-Path $logPath)) {
    $loaded = Select-String -LiteralPath $logPath -Pattern "Loading \[Expelliarmus 1.0.8\]" -Quiet
